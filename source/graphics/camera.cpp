...
void Camera::move(vmath::vec3 direction) { m_pos += m_speed * direction; }
...
vmath::vec3 Camera::getup() { return m_up; }
...
vmath::vec3 Camera::getPosition() { return m_pos; }
...
void Camera::point(float pX, float pY) {
  ...
  if (Nphi > 180.0f)
    ...
    m_phi = Nphi;
  float Ntheta =
      m_theta + angular_velocity * (pY - old_pY > 0 ? -0.25f : 0.25f);
  if (Ntheta > -80 && Ntheta < 80) m_theta = Ntheta;
  old_pX = pX;
  old_pY = pY;
}

vmath::vec3 Camera::calculateFront() {
  return normalize(vmath::vec3(sin(vmath::radians(m_phi)),
                               sin(vmath::radians(m_theta)),
                               -cos(vmath::radians(m_phi))));
}
...
vmath::mat4 Camera::pipeline() { return instric_matrix * getMatirxMatrix(); }
