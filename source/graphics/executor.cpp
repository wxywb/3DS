...
void Executor::init(const std::function<void(void)>& task) { task(); }

std::shared_ptr<Camera> Executor::get_camera() {
  return std::shared_ptr<Camera>(this->ptrcamera);
}

std::shared_ptr<Executor> init_graphics_executor(std::string name, size_t width,
                                                 size_t height) {
