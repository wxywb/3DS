...
Model::Model(std::string& name_) { filename = name_; }

void Model::Init() {
  m_importer = std::make_unique<Assimp::Importer>(*new Assimp::Importer());
  pScene = m_importer->ReadFile(
...
bool Model::InitfromScene(const aiScene* pScene) {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(texd_uvs[0]) * texd_uvs.size(),
               &texd_uvs[0], GL_STATIC_DRAW);
...
void Model::InitMesh(const aiMesh* pMesh, std::vector<vmath::vec3>& positions,
                     std::vector<vmath::vec3>& normals,
                     std::vector<vmath::vec2>& texd_uvs,
                     std::vector<unsigned int>& indices,
                     std::vector<vmath::vec3>& tangents) {
...
bool Model::InitMaterials(const aiScene* pScene, const std::string& Filename) {
  ...
  for (unsigned int i = 0; i < pScene->mNumMaterials; i++) {
    ...
    if (pMaterial->GetTextureCount(aiTextureType_NORMALS) > 0) {
      aiString Path;
      if (pMaterial->GetTexture(aiTextureType_NORMALS, 0, &Path, NULL, NULL,
...
void Model::LoadBones(size_t MeshIndex, const aiMesh* pMesh,
                      std::vector<VertexBoneData>& Bones) {
  for (size_t i = 0; i < pMesh->mNumBones; i++) {
    ...
    for (size_t j = 0; j < pMesh->mBones[i]->mNumWeights; j++) {
      size_t VertexID = m_meshes[MeshIndex].BaseVertex +
...
aiMatrix4x4 Model::BoneTransform(float TimeInSeconds,
...
void Model::CalcInterpolatedRotation(aiQuaternion& Out, float AnimationTime,
  ...
  const aiQuaternion& StartRoationQ =
...
void Model::CalcInterpolatedPositoin(aiVector3D& Out, float AnimationTime,
                                     const aiNodeAnim* pNodeAnim) {
  ...
  const aiVector3D& StartPositionQ =
      pNodeAnim->mPositionKeys[PositionIndex].mValue;
  const aiVector3D& EndPositionQ =
